...
mod d3d12_hello_frame_buffering {
    ...
    fn create_pipeline_state(
        ...
        let shaders_hlsl_path = asset_path.join("hello-frame-buffering-shaders.hlsl");
    ...
    fn move_to_next_frame(resources: &mut Resources) {
        let signaled_value = resources.command_queue.enqueue_signal().unwrap();

        let frame = &mut resources.frames[resources.frame_index];

        frame.fence_value = signaled_value;

        resources.frame_index =
            unsafe { resources.swap_chain.GetCurrentBackBufferIndex() as usize };
        let frame = &resources.frames[resources.frame_index];

        resources
            .command_queue
            .wait_for_gpu(frame.fence_value)
            .unwrap();
    }
}

fn main() -> Result<()> {
    run_sample::<d3d12_hello_frame_buffering::Sample>()?;
